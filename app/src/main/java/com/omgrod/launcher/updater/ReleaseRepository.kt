package com.omgrod.launcher.updater

import com.omgrod.launcher.utils.DownloadUtils.executeCoroutine
...
class ReleaseRepository(private val httpClient: OkHttpClient) {
    ...
    suspend fun getLatestLauncherRelease(): DownloadableLauncherRelease? {
        val releasePath = "$GITHUB_API_BASE/repos/Rod-GDPS/geode-android/releases/latest"
    ...
    suspend fun getReleaseByTag(tag: String): DownloadableGitHubLoaderRelease? {
        val releasePath = "$GITHUB_API_BASE/repos/Rod-GDPS/geode-android/releases/tags/$tag"
        val url = URL(releasePath)
    ...
    suspend fun getLatestGeodePreRelease(): DownloadableGitHubLoaderRelease? {
        val releasesUrl = "$GITHUB_API_BASE/repos/Rod-GDPS/geode-android/releases?per_page=2"
